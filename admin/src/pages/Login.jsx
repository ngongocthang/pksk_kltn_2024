...
const Login = () => {
  ...
  return (
    <form
      ...
      className="min-h-[80vh] flex items-center bg-white"
    >
      <div className="flex flex-col gap-3 m-auto items-start p-8 min-w-[340px] sm:min-w-96 border rounded-xl text-[#5E5E5E] text-sm shadow-lg">
        <p className="text-2xl font-semibold m-auto">
          {" "}
          Đăng nhập <span className="text-[#0091a1]"> {state}</span>
        </p>
        ...
        <button
          className={`bg-[#0091a1] text-white w-full py-2 rounded-md text-base ${
            loading ? "opacity-50 cursor-not-allowed" : ""
          }`}
          disabled={loading} // Vô hiệu hóa nút khi loading
        >
          {loading ? "Đang đăng nhập..." : "Đăng nhập"}{" "}
          {/* Hiển thị nội dung tùy thuộc vào trạng thái loading */}
        </button>
        {state === "Admin" ? (
          <p>
            Đăng nhập bác sĩ?{" "}
            <span
              className="text-[#0091a1] underline cursor-pointer"
